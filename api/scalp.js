  ...
  const out=(xs,type)=>{
    ...
    for(const x of xs){
      ...
      const extreme=type==="high"
        ? Math.max(...before.map(z=>z.high),...after.map(z=>z.high),-Infinity)
        : Math.min(...before.map(z=>z.low),...after.map(z=>z.low),Infinity);
      const prom=type==="high"?x.price-Math.min(candles[x.index].low,extreme):Math.max(candles[x.index].high,extreme)-x.price;
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(h1.length<50||m15.length<50||m5.length<50)throw new Error("Insufficient closed candle data");
