...
export default async function handler(req, res) {
  ...
  try {
    ...
    const h1Breakout = detectH1Breakout(h1, h1a, m15a, price, liquidity, cfg);
    const swing = buildSwingSignal(h1Breakout, h1a, m15a, price, liquidity, cfg);
    ...
    return json(res, 200, {
      ...
      swingConfidence:swing.confidence,
      swingStrength:swing.strength,
      swingReasons:swing.reasons,
      swingTradePlan:swing.tradePlan,
      swingEngine:swing.engine,
...
function detectH1Breakout(h1,h1a,m15a,price,liq,cfg){
  const closed=h1.slice(0,-1);
  if(closed.length<30) return {active:false,direction:"WAIT",type:"NONE",eventId:null,breakoutLevel:null,breakoutPrice:null,close:null,time:null,body:null,range:null,atr:null,bodyATR:null,closeBeyondATR:null};
  const c=closed.at(-1), prev=closed.slice(0,-1);
  const p=pivots(prev,cfg.pivotLeft,cfg.pivotRight);
  const lastHigh=p.hi.at(-1), lastLow=p.lo.at(-1);
  const a=atr(closed,cfg.atrPeriod)||0;
  const up=!!lastHigh && c.close>lastHigh.price;
  const dn=!!lastLow && c.close<lastLow.price;
  const direction=up?"BUY":dn?"SELL":"WAIT";
  if(direction==="WAIT") return {active:false,direction:"WAIT",type:"NONE",eventId:null,breakoutLevel:null,breakoutPrice:c.close,close:c.close,time:c.time,body:Math.abs(c.close-c.open),range:c.high-c.low,atr:a,bodyATR:a?Math.abs(c.close-c.open)/a:null,closeBeyondATR:null,brokenSwingHigh:lastHigh||null,brokenSwingLow:lastLow||null};
  const level=up?lastHigh.price:lastLow.price;
  const body=Math.abs(c.close-c.open), range=c.high-c.low;
  const closeBeyond=Math.abs(c.close-level);
  const bodyATR=a?body/a:0;
  const closeBeyondATR=a?closeBeyond/a:0;
  const wickAgainst=up?Math.max(0,c.high-c.close):Math.max(0,c.close-c.low);
  const clean=range>0 ? body/range>=0.55 && wickAgainst/range<=0.35 : false;
  return {
    active:true,direction,type:up?"H1_BOS_UP":"H1_BOS_DOWN",
    eventId:`${c.time}|${direction}|${level.toFixed(5)}`,
    breakoutLevel:level,breakoutPrice:c.close,close:c.close,time:c.time,
    body,range,atr:a,bodyATR:+bodyATR.toFixed(3),closeBeyondATR:+closeBeyondATR.toFixed(3),
    cleanBreakout:clean,
    brokenSwingHigh:lastHigh||null,brokenSwingLow:lastLow||null,
    h1Bias:h1a.bias,m15Bias:m15a.bias,
    targetAbove:liq.nearestAbove,targetBelow:liq.nearestBelow
  };
}

function buildSwingSignal(b,h1,m15,price,liq,cfg){
  if(!b.active) return {signal:"WAIT",confidence:0,strength:"C",reasons:["No confirmed H1 breakout on the latest closed H1 candle"],tradePlan:emptySwingPlan(),engine:{rule:"Confirmed H1 candle close beyond prior H1 swing; no retest required"}};
  const dir=b.direction;
  const bull=dir==="BUY";
  const h1Trend=(bull?h1.trend==="BULLISH":h1.trend==="BEARISH");
  const h1Struct=(bull?h1.structure?.bias==="BULLISH":h1.structure?.bias==="BEARISH");
  const m15Align=(bull?m15.bias==="BULLISH":m15.bias==="BEARISH");
  const emaAlign=(bull?(b.close>h1.ema20&&h1.ema20>h1.ema50):(b.close<h1.ema20&&h1.ema20<h1.ema50));
  const rsiOk=Number.isFinite(h1.rsi) ? (bull?h1.rsi>=52&&h1.rsi<75:h1.rsi<=48&&h1.rsi>25) : false;
  const displacement=b.bodyATR>=0.70;
  const clean=!!b.cleanBreakout;
  const target=bull?b.targetAbove:b.targetBelow;
  const distance=Number.isFinite(target)?Math.abs(target-b.close):null;
  const risk=Math.max(b.atr*(cfg.swingSlATR||1.15),Math.abs(b.close-b.breakoutLevel)+b.atr*.20);
  const rr=distance!=null?distance/risk:null;
  const targetRoom=rr==null?true:rr>=(cfg.swingMinRR||1.5);
  const score=
    30 +
    (clean?15:0)+
    (displacement?15:0)+
    ...
    (emaAlign?7:0)+
    (rsiOk?5:0);
  const potential=score>=65 && targetRoom;
  const reasons=[
    `H1 ${b.type} confirmed at close ${b.close.toFixed(2)}`,
    clean?"Clean breakout candle":"Breakout candle has weaker wick/body structure",
    displacement?`Displacement ${b.bodyATR.toFixed(2)} ATR`:"Displacement below 0.70 ATR",
    h1Trend?"H1 EMA trend aligned":"H1 EMA trend not fully aligned",
    h1Struct?"H1 market structure aligned":"H1 structure not fully aligned",
    m15Align?"M15 confirms direction":"M15 is not aligned",
    targetRoom?(target!=null?`Liquidity target gives ~${rr.toFixed(2)}R room`:"No mapped target; room not blocked"):"Nearest mapped liquidity is too close for the minimum swing RR"
  ];
  const signal=potential?dir:"WAIT";
  const confidence=Math.max(0,Math.min(100,Math.round(score)));
  const strength=confidence>=85?"A+":confidence>=75?"A":confidence>=65?"B":"C";
  return {
    signal,confidence,strength,reasons,
    tradePlan:buildSwingTradePlan(signal,b,liq,cfg),
    engine:{rule:"H1 confirmed breakout close; NO RETEST",scoreModel:"breakout + displacement + H1 structure/trend + M15 confirmation + liquidity room",h1Bias:h1.bias,m15Bias:m15.bias}
  };
}

function emptySwingPlan(){return {active:false,direction:"WAIT",entry:null,breakoutLevel:null,stopLoss:null,tp1:null,tp2:null,tp3:null,rr:null,target:null,invalidation:"No qualified H1 breakout"};}

function buildSwingTradePlan(signal,b,liq,cfg){
  if(signal==="WAIT") return emptySwingPlan();
  const bull=signal==="BUY", entry=b.close, a=b.atr||1;
  const risk=Math.max(a*(cfg.swingSlATR||1.15),Math.abs(entry-b.breakoutLevel)+a*.20);
  const sl=bull?Math.min(b.breakoutLevel-a*.35,entry-risk):Math.max(b.breakoutLevel+a*.35,entry+risk);
  const r=Math.abs(entry-sl);
  const mapped=bull?b.targetAbove:b.targetBelow;
  const tp1=bull?entry+r:entry-r, tp2=bull?entry+2*r:entry-2*r;
  let tp3=bull?entry+3*r:entry-3*r;
  if(Number.isFinite(mapped)) tp3=bull?Math.max(tp2,mapped):Math.min(tp2,mapped);
  const rr=Math.abs(tp3-entry)/r;
  return {active:true,direction:signal,entry,breakoutLevel:b.breakoutLevel,stopLoss:+sl.toFixed(2),tp1:+tp1.toFixed(2),tp2:+tp2.toFixed(2),tp3:+tp3.toFixed(2),rr:+rr.toFixed(2),target:mapped??null,invalidation:bull?`H1 close back below ${sl.toFixed(2)}`:`H1 close back above ${sl.toFixed(2)}`,noRetest:true};
}
