export default async function handler(req, res) {
  ...
  try {
    ...
    const technicalSignal=m15Conf.bias==="BULLISH"&&m5Trig.direction==="BUY"?"BUY":
                 m15Conf.bias==="BEARISH"&&m5Trig.direction==="SELL"?"SELL":"WAIT";
    const news=await getNews(API_KEY,cache,CFG);
    const signal=news.blockSignal&&technicalSignal!=="WAIT"?"WAIT":technicalSignal;
    ...
    const confidence=calcConfidence(signal,h1Struct.structure,m15Conf,m5Trig,warning,news);

    return res.status(200).json({
      ok:true,version:"XAUUSDSNIPER-FULL-V3",symbol:CFG.symbol,
      livePrice,price:livePrice,livePriceSource,livePriceError,
      signal,technicalSignal,signalStrength:signal==="WAIT"?"WAIT":confidence.score>=80?"STRONG":"NORMAL",
      ...
      rules:{scalpRequiresM15M5Alignment:true,h1IsContextNotHardVeto:true,structuralSROnly:true,liveNewsRiskFilter:true,highImpactNewsCanPauseSignal:true},
...
function calcConfidence(dir,h1,m15,m5,w,news={}){
  let s=(m15.score+m5.score)/2;if(dir==="BUY"&&h1==="BULLISH")s+=8;if(dir==="SELL"&&h1==="BEARISH")s+=8;if(w.warning)s-=12;if(news.status==="HIGH_RISK")s-=15;if(news.status==="MEDIUM_RISK")s-=7;
...
async function getNews(key,cache,cfg){
  const now=Date.now(), ttl=300000;
  if(cache.news && now-cache.newsAt<ttl) return cache.news;
  try{
    const d=new Date();
    ...
    const url=`https://www.financecalendar.com/wp-json/fc/v1/calendar?from=${from}&to=${to}&limit=500`;
    ...
    const events=raw.map(normalizeNewsEvent).filter(x=>x&&x.time).filter(x=>x.currency==="USD"||/US|United States|Federal Reserve|FOMC|CPI|PCE|NFP|Payroll|Jobs|GDP|PPI|Retail|ISM|JOLTS|Jobless|Powell/i.test(`${x.name} ${x.title} ${x.category}`));
    const relevant=events.map(x=>({...x,minutesUntil:Math.round((new Date(x.time).getTime()-now)/60000)}).filter(x=>x.minutesUntil>=-180&&x.minutesUntil<=4320);
    const high=relevant.filter(x=>x.impact==="high" && x.minutesUntil>=-15 && x.minutesUntil<=30);
    const medium=relevant.filter(x=>x.impact==="medium" && x.minutesUntil>=-10 && x.minutesUntil<=15);
    const status=high.length?"HIGH_RISK":medium.length?"MEDIUM_RISK":"CLEAR";
    const out={status,blockSignal:high.length>0,source:"financecalendar.com",reason:high.length?"High-impact USD event is within the news-risk window":medium.length?"Medium-impact USD event is within the news-risk window":"No relevant USD high/medium impact event in the active window",items:relevant.filter(x=>x.impact!=="low").sort((a,b)=>Math.abs(a.minutesUntil)-Math.abs(b.minutesUntil)).slice(0,12)};
    cache.news=out;cache.newsAt=now;return out;
  }catch(e){
    return {status:"UNKNOWN",blockSignal:false,source:"financecalendar.com",reason:e?.message||"Live news calendar unavailable",items:[]};
  }
}
function normalizeNewsEvent(x){
  if(!x||typeof x!=="object")return null;
  const time=x.time_utc||x.datetime||x.dateTime||x.time||x.date;
  return {name:x.name||x.title||x.event||"Economic event",title:x.title||x.name||null,time,impact:String(x.impact||x.importance||"low").toLowerCase(),currency:String(x.currency||x.country||"").toUpperCase(),category:x.category||null,consensus:x.consensus??null,prior:x.prior??null,actual:x.actual??null,url:x.url||null};
}
