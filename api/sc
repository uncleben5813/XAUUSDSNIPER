...
function analyzeTF(cs,tf,cfg){
  ...
  return {tf,bias:direction,structure:s,atr:a,ema20,ema50,last,time:cs.at(-1).time,
...
function buildScalpSignal(m15,m5,zones,liq,price,cfg){
  ...
  const sweep=detectSweep(m5,side);
  const displacement=detectDisplacement(m5,side,m5.atr||1);
