...
export default async function handler(req,res){
  ...
  try{
    ...
    const signalType=data.signalType||'NONE';
    if(signal==='WAIT') return res.status(200).json({ok:true,signal,notified:false});

    const price=Number(data.livePrice?.price ?? data.price ?? 0);
    const score=Number(data.score ?? 0);
    const key='xau_push_last_signal';
    const dedupe=`${signal}|${signalType}|${Math.round(price*100)}|${Math.floor(Date.now()/60000)}`;
    const r=redis();
    const last=await r.get(key);
    if(last===dedupe) return res.status(200).json({ok:true,signal,signalType,notified:false,deduped:true});
    ...
    for(const sub of list){
      try{ await webpush.sendNotification(sub,JSON.stringify({title,body,url:'/',signal,signalType})); sent++; alive.push(sub); }
      catch(e){ if(e.statusCode===404||e.statusCode===410) removed++; else alive.push(sub); }
    }
    await r.set('xau_push_subscriptions',alive);
    if(sent>0) await r.set(key,dedupe,{ex:3600});
    return res.status(200).json({ok:true,signal,signalType,notified:sent>0,sent,removed});
  }catch(e){
